	...
	$.niftyNoty = function(options){
		...
		iconTemplate = function(){
			...
			if (options && options.icon) {
				icon = '<div class="media-left"><span class="icon-wrap icon-circle alert-icon"><i class="'+ opt.icon +'"></i></span></div>';
			}
			return icon;
		},
		alertTimer,
	...
	bindSmallNav = function(){
		...
		$menulink.each(function(){
			...
			$el.popover({
				...
				$popover.css('padding-bottom','5px');

			}).on('shown.bs.popover', function () {
				if (!elHasSub && $listWidget.length == 0)return;

				var navbarHeight = function(){
					if(nifty.container.hasClass('navbar-fixed') || nifty.mainNav.hasClass('affix-top')){
						return '55px';
					}else{
						return '5px';
					}
				...
				var resetTop = $popover.css('top');

				$popover.css({'top':'','padding-bottom': '10px'});

				if( $popover.outerHeight() > ($(window).height() - 10 - (parseInt(navbarHeight)))){
					$popover.css({'top': navbarHeight, 'bottom' : '10px'});
				}else{
					if(nifty.document.scrollTop() < 55 && parseInt(resetTop) < 55){
						resetTop = '55px';
					}
					$popover.css('top', resetTop);
				}



				var oftop = $popover.height();
				if($poptitle.length && $poptitle.is(':visible')) oftop -= $poptitle.outerHeight();



				$popcontent.css({'height': oftop, 'min-width':'220px'}).children().addClass('nano').nanoScroller({
					preventPageScrolling : true
				});
				...
				function(){
					...
					hidePopover = setInterval(function(){
						if ($popover) {
							...
							nifty.window.one('scroll', function(){
								$el.removeClass('hover').popover('hide');
							});
							if(!$popover.is(":hover")){
								$el.removeClass('hover').popover('hide');
